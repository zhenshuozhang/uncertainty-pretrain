...
def main():
    ...
    subset_size = total_size // subset_num
    for i in range(subset_num):
    # 计算每个子集的起始和结束索引
        start = i * subset_size
        end = start + subset_size if i < subset_num - 1 else total_size
        subset_index = range(start, end)
        # 创建子集
        subsets_index.append(list(range(start, end)))
    ...
    file = f'./select_stat/result_train_{args.dataset}_{iter_num}_{select_num}_{subset_num}.log'
    result = dict()
    ...
    for i in range(iter_num):
        ...
        if args.order == 'random':
            ...
            for s in range(subset_num):
                subset_index = subsets_index[s]
                random.shuffle(subset_index)
                subset = Subset(pre_dataset, subset_index)
                subset_loader = DataLoader(subset, batch_size=args.pre_batch_size, num_workers = args.num_workers, shuffle=False)
                
                t1, t2, t3, mean_mmd2 = uni_kernel_all_all_stat(GNN_DKL, train_loader, subset_loader, 0, args)
                subset_mmd.append(mean_mmd2)

            # subset selection: select & merge
            subset_order = np.argsort(subset_mmd)
            ordered_subsets_index = [subsets_index[x] for x in subset_order]
            # max to min
            
            if args.order == 'reverse':
                select_index = sum(ordered_subsets_index[-select_num:], [])
        ...
        select_subset = Subset(pre_dataset, select_index)
        ...
        with open(file, 'a+') as f:
            ...
            #f.write(f'  subset order: dkl result: {subset_order}\n')
            f.write('\n')

        # train
        for e in range(iter_epochs):
            print(f"====iter {i} ====epoch " + str(e))
            train_acc, train_loss = train_subset(args, model, device, select_subset, optimizer)
    
    torch.save(gnn.state_dict(), f"./models_graphcl/graphcl_select_{args.dataset}_{iter_num}_{select_num}_order.pth")
