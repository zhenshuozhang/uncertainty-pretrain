...
def main():
    ...
    file = f'./ete_stat/result_train_{args.dataset}.log'
    result = dict()

    model_none = GNN_proj_graphpred(args.num_layer, args.emb_dim, args.proj_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type).to(device)
    model_pre = GNN_proj_graphpred(args.num_layer, args.emb_dim, args.proj_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type).to(device)
    #model = GNN_graphpred(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type)
    if not args.input_model_file == "none":
        backbone = 'GraphCL'
        model_pre.from_pretrained(args.input_model_file)
    else:
        backbone = 'nopretrain'

    if task_type == 'cls':

        none_y_pred, none_y_var, none_y_true, none_GNN_DKL, none_nn_acc = embsvdkl(model_none, train_loader, val_loader, test_loader, device, args)
        result_metrics_dict = classification_metrics(none_y_pred, none_y_true, note=f'{args.dataset}_embdkl')
        result['none_nn_acc'] = none_nn_acc
        result['none_acc'] = result_metrics_dict['roc-auc']['macro-avg']

        y_pred, y_var, y_true, GNN_DKL, nn_acc = embsvdkl(model_pre, train_loader, val_loader, test_loader, device, args)
        ...
        none_y_pred, none_y_var, none_y_true, none_GNN_DKL, none_nn_acc = embsvdkl(model_none, train_loader, val_loader, test_loader, device, args)
        result_metrics_dict = regression_metrics(none_y_pred, none_y_var, none_y_true)
        result['none_nn_acc'] = none_nn_acc
        result['none_acc'] = result_metrics_dict['rmse']['macro-avg']
        
        

    # same kernel in train data
    none_diversity_nk, pre_diversity_nk, none_pre_nk, none_pre_mmd_nk = uni_deep_kernel_uni_stat(model_none, model_pre, none_GNN_DKL, train_loader, 0, args, sample=False)
    none_diversity_pk, pre_diversity_pk, none_pre_pk, none_pre_mmd_pk = uni_deep_kernel_uni_stat(model_none, model_pre, GNN_DKL, train_loader, 0, args, sample=False)
    # same encoder
    none_train_diversity_nk, none_pretrain_diversity_nk, none_pretrain_nk, none_pretrain_mmd_nk = uni_kernel_all_all_stat(none_GNN_DKL, train_loader, pretrain_loader, 0, args)
    pre_train_diversity_pk, pre_pretrain_diversity_pk, pre_pretrain_pk, pre_pretrain_mmd_pk = uni_kernel_all_all_stat(GNN_DKL, train_loader, pretrain_loader, 0, args)
     
    with open(file, 'a+') as f:
        f.write(f'dataset: {args.dataset}\n distance type: {args.dis_type}\n')
        f.write(f' epoch: none\n')
        f.write(f'  none    : dkl result: {result["none_acc"]}, nn result: {result["none_nn_acc"]}\n')
        f.write(f'  pretrain: dkl result: {result["acc"]}, nn result: {result["nn_acc"]}\n')
        f.write(f' same kernel in train data:\n')
        f.write(f'  none-kernel: none_diversity: {none_diversity_nk}, pre_diversity: {pre_diversity_nk}, none_pre_distance: {none_pre_nk}, none_pre_mmd: {none_pre_mmd_nk}\n')
        f.write(f'  pre-kernel : none_diversity: {none_diversity_pk}, pre_diversity: {pre_diversity_pk}, none_pre_distance: {none_pre_pk}, none_pre_mmd: {none_pre_mmd_pk}\n')
        f.write(f' same encoder in different data:\n')
        f.write(f'  none-encoder: downstream_diversity: {none_train_diversity_nk}, pretrain_diversity: {none_pretrain_diversity_nk}, downstream_pretrain_distance: {none_pretrain_nk}, downstream_pretrain_mmd: {none_pretrain_mmd_nk}\n')
        f.write(f'  pre-encoder : downstream_diversity: {pre_train_diversity_pk}, pretrain_diversity: {pre_pretrain_diversity_pk}, downstream_pretrain_distance: {pre_pretrain_pk}, downstream_pretrain_mmd: {pre_pretrain_mmd_pk}\n')
        f.write('\n')
